...
\begin{proof}
	...
	Para eso sean $v_{i_1}, \dots, v_{i_k} \in T$ y $\lambda_{i_1},  \dots, \lambda_{i_k} \in \F$ tales que $\Bigsum{k}{\lambda_{i_k} v_{i_k}} = 0$. Como son finitos existe $k_0 \in \N$ tal que $v_i \in S_{k_0}$ para todo $i$, que al ser un conjunto linealmente independiente resulta que $\lambda_{1} = \lambda_{2} = \dots = \lambda_{i_n} = 0$. Conclu\'imos que $T \in P$, luego por \ref{Lema de Zorn} existe $M \in P$ elemento maximal.
...
\begin{definition}
	...
	A su vez decimos que son \textit{ortonormales} si son ortogonales y $\norm{x} = \norm{y} = 1$
...
\begin{example}
	El conjunto $\sett{e^{inx} \ , \ n \in \N \ , \ x \in [0,2 \pi]}$ es ortogonal.
\end{example}
...
\begin{proof}
	\begin{enumerate}
		\item Sean $u_1,\dots,u_n \in S$ y sea $z = x - \sum\limits_{i = 1}^{n}{{\ip{x,u_i}}u_i}$, luego:
	
	\[
	\begin{aligned}
		0 \leq & \ip{z,z} \\
		= & \ip{x - \sum\limits_{i = 1}^{n}{\ip{x,u_i}u_i},x - \sum\limits_{i = 1}^{n}{\ip{x,u_i}}u_i} \\
		= & \norm{x}^2 + \norm{\sum\limits_{i = 1}^{n}{\ip{x,u_i}u_i}}^2 - 2 \mathcal{R} \left(\ip{\sum\limits_{i = 1}^{n}{\ip{x,u_i}u_i},x}\right) \\
...
\begin{proof}
	...
	Finalmente, notemos que $\ip{S_k,u_j} = c_j$ para todo $k \geq j$ y ,adem\'as si $(c_n) \in l^2$, entonces $S_k \rightarrow \Bigsum{n \in \N}{c_n u_n}=:x$; por lo tanto por \ref{Prod interno es continuo} $c_n = \ip{S_k,u_n} \rightarrow \ip{x,u_n}$. 
	\qed
...
\begin{proof}
	Si $m \in M$ y $(x_n) \subset \ortogonal{M}$ es tal que $x_n \rightarrow x$ entonces por \ref{Prod interno es continuo}, $0 = \ip{m,x_n} \rightarrow \ip{m,x}$, por lo que $x \in \ortogonal{M}$. \qed
\end{proof}
...
\begin{theorem}
	\label{En dimension infinita la bola cerrada no es compacta}
...
\begin{proof}
	...
	By Zdzislaw Denkowski, Stanislaw Migórski, Nikolaos S. Papageorgiou]. En el caso reflexivo es trivial por usar \ref{Separable si y solo si la bola dual es metrizable estrella} en $\ball \ddual{X} = \ball X$. \qed
\end{proof}

\begin{corollary}
	\label{Reflexivo y separable implica bola es un espacio metrico compacto en la topologia debil}
	Sea $X$ Banach reflexivo y separable, entonces $\left(\ball X, wk\right)$ es un espacio m\'etrico compacto
\end{corollary}

\begin{proof}
	Por \ref{Teorema de Kukatani}, $\ref{Dual es separable si y solo si la bola es debil metrizable}$ y \ref{X es reflexivo y separable si y solo si dual es reflexivo y separable} \qed
\end{proof}
...
\begin{definition}
	Sean $X,Y$ Banach y $T: X \rightarrow Y$ lineal, entonces $T$ es un \textit{operador compacto} si $\overline{T(\ball X)}$ es compacto en $Y$
\end{definition}

\begin{proposition}
	Sean $X,Y$ Banach y $T: X \rightarrow Y$ lineal y compacto, entonces $T \in L(X,Y)$
\end{proposition}

\begin{proof}
	Supongamos que $T$ no es acotado, entonces existe $\sett{x_n}_{n \in \N} \subset \ball X$ tal que $\norm{T(x_n)}_Y \rightarrow \infty$, luego $\sett{T(x_n)}_{n \in \N} \subset \overline{T(\ball X)}$ no admite subsucesi\'on convergente; conclu\'imos que $T \in L(X,Y)$. \qed
\end{proof}

\begin{definition}
	Sea $X,Y$ Banach, notamos $L_0(X,Y) = \sett{T \in L(X,Y) \tq T \text{ es compacto}}$
\end{definition}

\begin{proposition}
	\label{Los operadores compactos son cerrados}
	Sean $X,Y$ Banach, luego $L_0(X,Y)$ es cerrado en $L(X,Y)$
\end{proposition}

\begin{proof}
	Sea $\sett{T_n}_{n \in \N} \subset L_0(X,Y)$ tal que $T_n \xrightarrow{\norm{.}_{L_0(X,Y)}} T \in L(X,Y)$ y veamos que $T$ es compacto; para eso sea $\epsilon > 0$ y $n_0$ tal que $\norm{T-T_{n_0}} < \epsilon$. Como $T_{n_0}$ es compacto existen $x_1, \dots, x_m \in \ball X$ tal que $T_{n_0}\left(\ball X\right) \subset \Bigcup{1 \leq j \leq m}{B_{\frac{\epsilon}{3}}(T_{n_0}(x_j))}$. Luego si $x \in \ball X$ y tomamos el $x_j$ tal que $\norm{T_{n_0}(x_j) - T_{n_0}(x)}_Y < \frac{\epsilon}{3}$:
	
	\begin{equation*}
		\begin{aligned}
			\norm{T(x_j) - T(x)}_Y \leq & \norm{T(x_j) - T_{n_0}(x_j)}_Y + \norm{T_{n_0}(x_j) - T_{n_0}(x)}_Y + \norm{T_{n_0}(x) - T(x)}_Y \\
			< & 2 \norm{T-T_n} + \frac{\epsilon}{3} \\
			< & \epsilon
		\end{aligned}
	\end{equation*}
	
	Luego $T\left(\ball X\right) \subset \Bigcup{1 \leq j \leq m}{B_{\epsilon}(T(x_j))}$ y $T\in L_0(X,Y)$. \qed
	
\end{proof}

\begin{proposition}
	Sean $X,Y$ Banach, luego $L_0(X,Y)$ es un $L(X,Y)$ ideal bil\'atero, o sea si $A \in L(X)$, $B \in L(Y)$ y $T \in L_0(X,Y)$ entonces $TA, BT \in L_0(X,Y)$
\end{proposition}

\begin{proof}
	Sea $\sett{x_n}_{n \in \N} \in \ball X$, luego como $T$ es compacto existe $\sett{x_{n_k}}_{k \in \N}$ subsucesi\'on tal que $T(x_{n_k}) \xrightarrow{\norm{.}_Y} y \in Y$ y como $B \in L(Y)$ entonces $BT(x_{n_k}) \xrightarrow{\norm{.}_Y} By \in Y$; por lo tanto $BT \in L_0(X,Y)$.
	
	Rec\'iprocamente, si $\sett{x_n}_{n \in \N} \subset \ball X$ entonces $\sett{\frac{A(x_n)}{\norm{A}}}_{n \in \N} \subset \ball X$, como $T$ es compacto existe $\sett{\frac{A(x_{n_k})}{\norm{A}}}_{k \in \N}$ subsucesi\'on tal que $T \left(\frac{A(x_{n_k})}{\norm{A}} \right) = \frac{1}{\norm{A}} TA(x_{n_k}) \xrightarrow{\norm{.}_Y} y \in Y$; por lo tanto $TA(x_{n_k}) \xrightarrow{\norm{.}_Y} \norm{A}y$ y $TA$ es compacto. \qed
\end{proof}

\begin{definition}
	Sea $X,Y$ Banach, notamos $L_{00}(X,Y) = \sett{T \in L(X,Y) \tq \dim \left(\rank T\right) < \infty}$
\end{definition}

\begin{definition}
	Sean $X,Y$ Banach y $T\in L(X,Y)$ entonces $T$ es \textit{completamente continuo} si para toda $\sett{x_n}_{n \in \N} \subset X$ tal que $x_n \xrightarrow{d} x$ se sigue que $T(x_n) \xrightarrow{\norm{.}_Y} T(x)$
\end{definition}

\begin{proposition}
	\label{Compacto implica completamente continuo y la vuelta si es reflexivo X}
	Sean $X,Y$ Banach y $T \in L(X,Y)$, entonces:
	
	\begin{enumerate}
		\item Si $T$ es compacto entonces es completamente continuo
		\item Si $T$ es completamente continuo y $X$ es reflexivo entonces $T$ es compacto
		\item La condici\'on de reflexividad en $X$ es necesaria
	\end{enumerate}
...
\begin{proof}
	...
	\begin{enumerate}
		\item Sea $\sett{x_n}_{n \in \N} \subset X$ tal que $x_n \xrightarrow{d} 0$, luego por \ref{Resultados de convergencia debil} podemos suponer sin p\'erdida de generalidad que $\sup\limits_{n} \norm{x_n}_X \leq 1$. Como $T$ es compacto existe $\sett{x_{n_k}}_{k \in \N} \subset \sett{x_n}_{n \in \N}$ subsucesi\'on e $y \in Y$ tal que $T(x_{n_k}) \xrightarrow{\norm{.}_Y} y$; pero $x_{n_k} \xrightarrow{d} 0$ y por \ref{T es fuerte-fuerte continua entonces es debil-debil continua} $T \in L((X,wk), (Y,wk)$ por lo que $T(x_{n_k}) \xrightarrow{d} T(0) = 0$, por \ref{Topologia debil es Hausdorff} conclu\'imos que $y = 0$. Como $0$ es el \'unico punto l\'imite de $\sett{T(x_n)}_{n \in \N}$ en un compacto, conclu\'imos que $T(x_n) \xrightarrow{\norm{.}} 0$.
		
		\item Supongamos adem\'as que $X$ es separable, entonces por \ref{Reflexivo y separable implica bola es un espacio metrico compacto en la topologia debil} sabemos que $\left(\ball X, wk\right)$ es un espacio m\'etrico compacto. Si $\sett{x_n}_{n \in \N} \subset \ball X$ es una sucesi\'on,  existe $x \in X$ y $\sett{x_{n_k}}_{k \in \N} \subset \sett{x_n}_{n \in \N}$ subsucesi\'on tal que $x_{n_k} \xrightarrow{d} x$; luego $T(x_{n_k}) \xrightarrow{\norm{.}_Y} T(x)$ pues $T$ es completamente continuo. Conclu\'imos que $T(\ball X)$ es secuencialmente compacto y entonces $T$ es compacto.
		
		Si $X$ es arbitrario y $\sett{x_n}_{n \in \N} \subset \ball X$ es una sucesi\'on entonces $X_1 = \overline{\ip{\sett{x_n}}_{\mathbb{F}}}^{\norm{.}_X}$ es un espacio de Banach separable (trivial) y reflexivo por \ref{Subespacio de Reflexivo es reflexivo}. Sea $T_1 = T \vert_{X_1}$ y notemos el siguiente lema:
		
		\begin{lemma}
			\label{Lemma: Compacto implica completamente continuo y la vuelta si es reflexivo X}
			Sea $X,Y$ Banach y $T \in L(X,Y)$ completamente continuo, luego si $M \subseteq X$ es subespacio cerrado $T \vert_M: M \rightarrow \overline{T(M)}^{\norm{.}_Y}$ es completamente continuo.
		\end{lemma}
		
		\begin{proof}
			Sea $\sett{x_n}_{n \in \N} \subset M$ tal que $x_n \xrightarrow{d} x$, luego como $T$ es completamente continuo $T(x_n) \xrightarrow{\norm{.}_Y} T(x)$. Como $\norm{.}_{\overline{T(M)}^{\norm{.}_Y}} = \norm{.}_Y \vert_{\overline{T(M)}^{\norm{.}_Y}}$ entonces $T \vert_{M}(x_n) = T(x_n) \xrightarrow{\norm{.}_{\overline{T(M)}^{\norm{.}_Y}}} T(x) = T \vert_{M}(x)$ y $T\vert_M$ es completamente continuo. \qed
		\end{proof}
		
		Luego por \ref{Lemma: Compacto implica completamente continuo y la vuelta si es reflexivo X} $T_1$ es completamente continuo, por ende compacto por el par\'agrafo anterior. Como $\sett{T(x_n)}_{n \in \N} = \sett{T_1(x_n)}_{n \in \N}$ conclu\'imos que $T$ es compacto.
		
		\item Sea $X = Y = l^1$ un espacio separable no reflexivo y notemos el siguiente lema:
		
		\begin{lemma}
			\label{Toda sucesion que converge debilmente en l1 converge fuertemente}
			Sea $\sett{x_n}_{n \in \N} \subset l^1$ una sucesi\'on tal que $x_n \xrightarrow{d} x$, luego $x_n \xrightarrow{\norm{.}_{l^1}} x$.
		\end{lemma}
		
		\begin{proof}[Demostraci\'on del lema]
			Recordemos que $\dual{l^1} = l^{\infty}$ y por \ref{Teorema de Alaoglu} y \ref{Separable si y solo si la bola dual es metrizable estrella} tenemos que $\left(\ball l^{\infty}, \dual{wk}\right)$ es un espacio m\'etrico compacto, en particular completo.
			
			Sea $\sett{x_n}_{n \in \N} \subset l^1$ una sucesi\'on tal que $x_n \xrightarrow{d} 0$ y $\epsilon > 0$, para cada $m \in \N$ definamos:
			
			\begin{equation*}
				F_m = \sett{\phi \in \ball l^{\infty} \tq \abs{\ip{\phi, x_n}} \leq \frac{\epsilon}{3} \quad \forall n \geq m}
			\end{equation*}
			
			Luego $F_n$ es $\dual{wk}$ cerrado en $\ball l^{\infty}$ y $\ball l^{\infty} = \Bigcup{m \in \N}{F_m}$; por el teorema de Baire existe $m_0$ tal que $\interior{F_m} \neq \emptyset$ para la topolog\'ia $\dual{wk}$. Luego existe $\phi_0$ y $\delta > 0$ tal que $B = B_{\delta}^{\dual{wk}}(\phi_0) \subset F_{m_0}$; sea $J \geq 1$ tal que $2^{-(J-1)} < \delta$ y fijemos $n  \geq m_0$, luego podemos definir:
			
			\begin{equation*}
				\psi = \left\lbrace \begin{array}{cc}
					\psi(j) = \phi_{m_0}(j) & \text{si } 1 \leq j \leq J \\
					\psi(j) = sign\left(x_n(j)\right) & \text{si } j > J 
				\end{array}\right.
			\end{equation*}
			
			Es claro que $\psi \in \ball l^{\infty}$ y $\psi(j)x_n(j) = \abs{x_n(j)}$ para todo $j > J$; es m\'as, afirmo que $\psi \in B \subset F_{m_0}$ por lo que $\abs{\ip{\psi, x_{n}}} \leq \frac{\epsilon}{3}$ para todo $n \geq m_0$. Por otro lado como $x_n \xrightarrow{d} 0$ existe $m_1 \geq m_0$ tal que $\sum_{j=1}^{J}{\abs{x_n(j)}} < \frac{\epsilon}{3}$ y juntando todo:
			
			\begin{equation*}
			\begin{aligned}
				\norm{x_n}_{l^1} = & \Bigsum{j \in \N}{\abs{x_n(j)}} \\
				< & \frac{\epsilon}{3} + \abs{\underbrace{\Bigsum{j > J+1}{\abs{x_n(j)}} + \Bigsum{1 \leq j \leq J}{\phi_{m_0}x_n(j)}}_{\ip{\psi, x_{n}}}} + \abs{ \Bigsum{1 \leq j \leq J}{\phi_{m_0}x_n(j)}} \\
				< & \frac{2\epsilon}{3} +  \abs{ \Bigsum{1 \leq j \leq J}{x_n(j)}} \\
				< & \epsilon
			\end{aligned}
			\end{equation*}
			
			Para todo $n \geq m_1$ por lo que $x_n \xrightarrow{\norm{.}_{l^1}} 0$. \qed
			
		\end{proof}
		
		Por lo tanto si $\sett{x_n}_{n \in \N} \subset l^1$ es tal que $x_n \xrightarrow{d} x$, por \ref{T es fuerte-fuerte continua entonces es debil-debil continua} $T(x_n) \xrightarrow{d} T(x)$ y por \ref{Toda sucesion que converge debilmente en l1 converge fuertemente} $T(x_n) \xrightarrow{\norm{.}_{l^1}} T(x)$ por lo que todo $T \in L(l^1,l^1)$ es completamente continuo; basta probar que existen operadores no compactos all\'i.
		
		En efecto, $id : l^1 \rightarrow l^1$ es continuo pero no compacto, pues $id(\ball l^1) = \ball l^1$ que no es compacto por \ref{En dimension infinita la bola cerrada no es compacta}. \qed
...
\begin{theorem}[Teorema de Schauder]
	\label{Teorema de Schauder}
	Sean $X,Y$ Banach y $T \in L(X,Y)$, luego $T$ es compacto si y s\'olo si $\dual{T}$ es compacto
\end{theorem}

\begin{proof}
	Sea $\sett{f_n}_{n \in \N} \subset \ball \dual{Y}$ una sucesi\'on, por \ref{Teorema de Alaoglu} existe $f \in \ball \dual{Y}$ tal que $f_n \xrightarrow[\dual{d}]{cl} f$. Sea $\epsilon > 0$ y $N \geq 1$, como $T$ es compacto existen $y_1, \dots, y_n \in Y$ tal que  $T(\ball X) \subset \Bigcup{1 \leq i \leq n}{B_{\frac{\epsilon}{3}}^{\norm{.}_Y}(y_i)}$ ; sea $x \in \ball X$ y $k_0$ tal que $\norm{T(x) - y_{k_0}}_Y < \frac{\epsilon}{3}$, luego:
	
	\begin{equation*}
	\begin{aligned}
		\abs{\ip{\dual{T}(f) - \dual{T}(f_n), x}} = & \abs{\ip{f-f_n, T(x)}} \\
		\leq & \abs{\ip{f-f_n, T(x) - y_{k_0}}} + \underbrace{\abs{\ip{f-f_n, y_{k_0}}}}_{\rightarrow 0} \\
		 \leq & 2 \norm{T(x) - y_k}_Y + \frac{\epsilon}{3} \\
		 < \epsilon
	\end{aligned}
	\end{equation*}
	
	Por lo tanto $\norm{\dual{T}(f) - \dual{T}(f_n)}_{\dual{X}} = \sup\limits_{x \in \ball X} \abs{\ip{\dual{T}(f) - \dual{T}(f_n), x}} \leq \epsilon$. En resumen probamos que si $\sett{f_n}_{n \in \N} \subset \ball \dual{Y}$ entonces $\sett{\dual{T}(f_n)}_{n \in \N} \subset \dual{X}$ tiene una subsucesi\'on $\norm{.}_{\dual{X}}$ convergente y por ende $\dual{T}$ es compacto.
	
	
	Rec\'iprocamente, si $\dual{T}$ es compacto entonces por lo anterior $\ddual{T}$ lo es. Notemos:
	
	\begin{lemma}
		\label{Lemma: Teorema de Schauder}
		Sea $X,Y$ Banach y $T \in L(X,Y)$ compacto, luego si $M \subseteq X$ es subespacio cerrado $T \vert_M: M \rightarrow Y$ es compacto.
	\end{lemma}
	
	\begin{proof}
		Sea $\sett{x_n}_{n \in \N} \subset \ball M$ tal que $x_n \xrightarrow{d} x$, luego como $T$ es compacto existe $\sett{x_{n_k}}_{k \in \N} \subset \sett{x_n}_{n \in \N}$ subsucesi\'on e $y \in Y$ tal que $T(x_{n_k}) \xrightarrow{\norm{.}_Y} y$. Como $\norm{.}_{\overline{T(M)}^{\norm{.}_Y}} = \norm{.}_Y \vert_{\overline{T(M)}^{\norm{.}_Y}}$ entonces $T \vert_{M}(x_n) = T(x_n) \xrightarrow{\norm{.}_{\overline{T(M)}^{\norm{.}_Y}}} T(x) = T \vert_{M}(x)$ y $T\vert_M$ es compacto. \qed
	\end{proof}
	
	Aplicando \ref{Lemma: Teorema de Schauder} a $X \subseteq \ddual{X}$ y por \ref{Propiedades del operador adjunto} resulta que $T = \ddual{T} \vert_{X}$ es compacto. \qed
	
\end{proof}

\subsection{Ecuaci\'on integral de Fredholm}

\begin{proposition}
	Sea $[a,b] \subset \R$ y $X = \C[a,b]$, asimismo sea $k: [a,b]^2 \rightarrow \R$ continua; luego si definimos $K : X \rightarrow X$
	
	\begin{equation*}
		K(\phi)(s) = \int_{a}^{b} k(s,t)\phi(t) dt
	\end{equation*}
	
	Resulta un operador compacto.
	
\end{proposition}

\begin{proof}
	Es claro que $K$ es lineal y acotado, veamos directamente que es compacto; para esto sea $\sett{\phi_n}_{n \in \N} \subset C[a,b]$ tal que $\norm{\phi_n}_{C[a,b]} \leq 1$ y veamos que $\left(K(\phi_n)\right)_{n \in \N} \subset K(\ball C[a,b])$ admite una subsucesi\'on convergente. Recordemos:
	
	\begin{theorem}[Teorema de Arzela-Ascoli]
		\label{Arzela Ascoli}
		Si $X$ es compacto y $\F \subset C(X)$ entonces $\F$ es totalmente acotado si y s\'olo si $\F$ es acotado y equicontinuo
	\end{theorem}
	
	Como $k$ es continua en un compacto, existe $M > 0$ tal que $\norm{k}_{C[a,b]} \leq M$ por lo que:
	
	\begin{equation*}
		\abs{K(\phi_n)(s)} \leq \int_{a}^{b}{\abs{k(s,t)}\abs{\phi_n(t)} dt} \leq M(b-a) \qquad \forall n \in \N
	\end{equation*}
	
	Y conclu\'imos que $\left(K(\phi_n)\right)_{n \in \N}$ es equiacotada.
	
	Adem\'as, dado $\epsilon > 0$, existe $\delta > 0$ tal que si $\norm{(s,t) - (s',t')}_{[a,b]^2} < \delta$ entonces $\abs{k(s,t) - k(s',t')} < \frac{\epsilon}{b-a}$; por lo tanto si $\abs{s-s'} < \delta$ entonces:
	
	\begin{equation*}
		\abs{K(\phi_n)(s) - K(\phi_n)(s')} \leq \int_{a}^{b}{\abs{k(s,t) - k(s',t)}\abs{\phi_n(t)}  dt} \leq \epsilon
	\end{equation*}
	
	Conclu\'imos que $\sett{K(\phi_n)}_{n \in \N}$ es equicontinua y equiacotada y por \ref{Arzela Ascoli} existe $\phi \in \overline{K(\ball C[a,b])}^{C[a,b]}$  y $\left(K(\phi_{n_k})\right) \subset \left(K(\phi_n)\right)$ subsucesi\'on tal que $K(\phi_{n_k}) \xrightarrow[\norm{.}_{C[a,b]}]{k \rightarrow \infty} \phi$. Luego $K$ es compacto. \qed
	
\end{proof}

\begin{proposition}
	\label{Identidad - T inversible}
	Sea $X$ Banach y $T\in \ball L(X)$, entonces $Id_X - T \in L(X)$ y $\left(Id -T \right) ^{-1} = \Bigsum{n \in \N}{T^n}$
\end{proposition}

\begin{proof}
	Notemos que $\norm{T^n} \leq \norm{T}^n \leq 1$ para todo $n \in \N$, luego:
	
	\begin{equation*}
		\Bigsum{n \in \N}{\norm{T^n}} \leq \Bigsum{n \in \N}{\underbrace{\norm{T}^n}_{< 1}} = \dfrac{1}{1-\norm{T}} < \infty
	\end{equation*}
	
	Luego por \ref{Banach si y solo si abs convergente es convergente} conclu\'imos que $\Bigsum{n \in \N}{T^n} \in L(X)$. Finalmente notemos que:
	
	\begin{equation*}
	\begin{aligned}
		Id = & \Bigsum{n \in \N}{T^n} - \Bigsum{n \in \N}{T^{n+1}} = \left(Id - T\right) \circ \Bigsum{n \in \N}{T^n} \\
		= & \Bigsum{n \in \N}{T^n} - \Bigsum{n \in \N}{T^{n+1}} = \Bigsum{n \in \N}{T^n} \circ \left(Id - T\right) 
	\end{aligned}
	\end{equation*}
	.\qed
...
\begin{example}[Aplicaci\'on]
	Supongamos que $K$ es como antes con $\norm{k}_{C[a,b]} < \frac{1}{b-a}$, luego dada $g \in C[a,b]$, $g = I-T \left(\phi\right)$ si y s\'olo si $\phi = \Bigsum{n \in \N}{K^n(y)}$. 
\end{example}

\begin{corollary}
	Sea $X$ Banach y $T_0 \in L(X)$ tal que $T_0^{-1} \in L(X)$, luego si $T \in B^{L(X)}_{\frac{1}{\norm{T_0^{-1}}}} (T_0)$ entonces $T^{-1} \in L(X)$
\end{corollary}

\begin{proof}
	Notemos que $T = T_0 - \left(T - T_0\right) = T_0 \circ \left(I - \underbrace{T_0^{-1} \circ \left(T_0 - T\right)}_{\norm{T_0^{-1} \circ \left(T_0 - T\right)} < 1}\right)$, luego por \ref{Identidad - T inversible} el operador $\left(I - T_0^{-1} \circ \left(T_0 - T\right)\right)$ es invertible. AL ser $T$ composici\'on de invertibles, es invertible. \qed
\end{proof}

\newpage

\section{Operadores compactos en espacio de Hilbert}

\subsection{Propiedades y Ejemplos}

\begin{theorem}[Propiedad de aproximaci\'on en Hilbert]
	\label{En Hilbert un operador compacto es limite de rango finito}
	Sean $X,Y$ espacios de Hilbert, luego $T \in L_0(X,Y)$ si y s\'olo si existe $\sett{T_n}_{n \in \N} \subset L_{00}(X,Y)$ tal que $\norm{T-T_n} \rightarrow 0$
\end{theorem}

\begin{proof}
	Notemos el siguiente lema trivial:
	
	\begin{lemma}
		$L_{00}(X,Y) \subset L_0(X,Y)$
	\end{lemma}
	
	\begin{proof}[Demostraci\'on del lema ]
		Si $T \in L_{00}(X,Y)$ entonces $T \in L(X,Y)$ por lo que $T(\ball X)$ es acotada y finito dimensional, por ende precompacta; en conclusi\'on $T \in L_0(X,Y)$. \qed
	\end{proof}
	
	Por lo tanto $\sett{T_n}_{n \in \N} \subset L_0(X,Y)$ y por \ref{Los operadores compactos son cerrados} $T \in L_0(X,Y)$.
	
	Rec\'iprocamente como $\overline{T(\ball X)}^{Y}$ es compacto en particular es separable, luego $L = \overline{\rank T}^{Y}$ es separable. Sea $\B = \sett{e_1, \dots , e_n, \dots}$ una base ortonormal numerable de $L$ por \ref{Todo conjunto ortonormal en un separable es numerable},  $P_n = P_{\ip{e_1, \dots , e_n}}$ y $T_n = P_n \circ T \in L_{00}(X,Y)$ luego veamos el siguiente lema:
	
	\begin{lemma}
		\label{Lemma: En Hilbert un operador compacto es limite de rango finito}
		Si $x \in X$ entonces $T_n(x) \xrightarrow{\norm{.}_Y} Tx$
	\end{lemma}
	
	\begin{proof}[Demostraci\'on del lema]
		Sea $y = T(x)$, luego por \ref{Escritura de elemento de un HIlbert en un ortonormal completo} sabemos que $T_n(x) = P_n(y) \xrightarrow{\norm{.}_Y} y = T(x)$. \qed
	\end{proof}
		
	Como $T$ es compacto existen $x_1, \dots, x_m \in \ball X$ tal que $T\left(\ball X\right) \subset \Bigcup{1 \leq j \leq m}{B_{\frac{\epsilon}{3}}(T(x_j))}$. Luego si $x \in \ball X$ y tomamos el $x_j$ tal que $\norm{T(x_j) - T(x)}_Y < \frac{\epsilon}{3}$:
	
	\begin{equation*}
	\begin{aligned}
	\norm{T(x) - T_n(x)}_Y \leq & \norm{T(x) - T(x_j)}_Y + \norm{T(x_j) - T_{n}(x_j)}_Y + \norm{\underbrace{T_{n}(x_j) - T_n(x)}_{P_n \left(T(x) - T(x_j\right)}}_Y \\
	< & 2 \norm{T(x)-T(x_j)} + \norm{T(x_j) - T_{n}(x_j)}_Y \\
	< & \frac{2\epsilon}{3} + \underbrace{\norm{T(x_j) - T_{n}(x_j)}_Y}_{\xrightarrow{\ref{Lemma: En Hilbert un operador compacto es limite de rango finito}} 0 } 
	\end{aligned}
	\end{equation*}
		
	Luego existe $n_0 \in \N$ tal que para todo $n \geq n_0$ vale que $\norm{T-T_n} = \sup\limits_{x \in \ball X}{\norm{T(x) - T_n(x)}} < \epsilon$. \qed	
	
\end{proof}

\begin{corollary}
	\label{Corolario En Hilbert un operador compacto es limite de rango finito}
	Sean $X,Y$ Hilbert y $T \in L_0(X,Y)$, luego si $\B = \sett{e_1, \dots , e_n, \dots} \subset \overline{\rank T}^{Y}$ es una base ortonormal numerable y definimos $P_n = P_{\ip{e_1, \dots, e_n}}$ entonces $\norm{P_n \circ T - T} \rightarrow 0$. 
\end{corollary}

\begin{proposition}
	\label{Operador diagonalizable es compacto si autovalores tienden a cero}
	Sea $X$ Hilbert separable con base $\B$, $\sett{\alpha_n} \subset \mathbb{F}$ con $M = \sup\limits_{n \geq 1}{\abs{\alpha_n}} < \infty $y $T$ dado por $Te_n = \alpha_ne_n$. Luego existe $\tilde{T} \in L(X)$ tal que $\tilde{T} \vert_{\sett{e_n}} = T$ con $\norm{\tilde{T}} = M$. Es m\'as, $T \in L_0(X)$ si y s\'olo si $\alpha_n \rightarrow 0$.
\end{proposition}

\begin{proof}
	Es trivial la extensi\'on por linealidad y haremos el abuso de notaci\'on de llamar $T$ a la extensi\'on]; el hecho que $\norm{T} = M$ tambi\'en es claro por lo que sea $T_n = T - P_n \circ T$ donde $P_n$ es la misma de antes. Luego:
	
	\begin{equation*}
	T_n (e_j) = \left\lbrace \begin{array}{ccc}
	T(e_j) - T(e_j) & = 0 & \text{si } j < n \\
	T(e_j) - T(0) & = \alpha_j e_j & \text{si } j > n 
	\end{array}\right.
	\end{equation*}
	
	Por lo tanto $\norm{T_n} = \sup\limits_{j > n}{\abs{\alpha_n}}$.
	
	En conclusi\'on si $\alpha_n \rightarrow 0$ entonces $\norm{T_n} \rightarrow 0$ y $T \in L_0(X)$ por \ref{Los operadores compactos son cerrados}.
	
	Rec\'iprocamente si $T \in L_0(X)$ entonces por \ref{Corolario En Hilbert un operador compacto es limite de rango finito} sabemos que $\norm{T_n} \rightarrow 0$ por lo que $\alpha_n \rightarrow 0$. \qed
	
\end{proof}

\begin{proposition}
	Sea $\left(X,\Omega, \mu\right)$ un espacio de medida y sea $k \in L^2(X \times X, \Omega \times \Omega, \mu \times \mu)$, luego:
	\begin{equation*}
...
\begin{definition}
	Sea $X$ Hilbert y $T \in L(X)$, luego $\alpha \in \mathbb{F}$ es un \textit{autovalor de $T$} si $\ker(T - \alpha) \neq (0)$. Es m\'as si $0 \neq x \in \ker( T - \alpha) $ entonces $x$ es llamado un \textit{autovector de $T$} y cumple que $T(x) = \alpha x$.
	
	Notaremos $\sigma_p(T) = \sett{\alpha \in \mathbb{F} \tq \alpha \text{ es autovalor}}$
	
\end{definition}

\begin{proposition}
	\label{En un operador compacto los autoespacios son finito dimensionales}
	Sea $X$ Hilbert, $T \in L_0(X)$ y $\lambda \in \sigma_p(T)$, luego si $\lambda \neq 0$ entonces $\dim \left(\ker(T-\lambda) \right) < \infty$.
\end{proposition}

\begin{proof}
	Sea $\sett{e_n}_{n \in \N} \subset \ker(T-\lambda)$ una base ortogonal del autoespacio, como $T$ es compacto existe $\sett{e_{n_k}}_{k \in \N}$ subsucesi\'on tal que $T(e_{n_k}) \rightarrow x \in X$; pero $\norm{T(e_{n_k}) - T(e_{n_j})}^2_X = \abs{\lambda}^2 \norm{e_{n_j} - e_{n_k}}_X > 0$ si $n_j \neq n_k$ por lo que $\dim(\ker(T-\lambda))< \infty$. \qed
\end{proof}

\begin{proposition}
	\label{Condicion autovalor en Hilbert y compacto}
	Sea $X$ Hilbert y $T \in L_0(X)$, luego si $\lambda \neq 0$ cumple que $\inf\limits_{\norm{x}_X = 1}{\norm{(T-\lambda)x}_X} = 0$ entonces $\lambda \in \sigma_p(T)$
\end{proposition}

\begin{proof}
	Sea $\sett{x_n}_{n \in \N}$ con $\norm{x_n}_X = 1$ y $(T-\lambda)x_n \xrightarrow{\norm{.}_X} 0$, luego como $\sett{x_n}_{n \in \N} \subset ball X$ existe $y \in X$ y $\sett{x_{n_k}}_{k \in \N}$ subsucesi\'on tal que $T(x_{n_k}) \xrightarrow{\norm{.}_X} f$.
	
	Notemos que $x_{n_k} = \frac{1}{\lambda} \left[(\lambda -T)(x_{n_k}) + T(x_{n_k})\right] \rightarrow \frac{1}{\lambda}f$ por lo que $f \neq 0$ por un lado y, por el otro como $T$ es continua, $T(x_{n_k}) \rightarrow T\left(\frac{1}{\lambda}f\right)$; por unicidad del l\'imite $\frac{1}{\lambda}T(f) = f$ y conclu\'imos que $\lambda \in \sigma_p(T)$. \qed
	 
\end{proof}

\begin{corollary}
	Sea $X$ Hilbert,  $T \in L_0(X)$, $\lambda \neq 0 \not \in \sigma_p(T)$ tal que $\overline{\lambda} \not \in \sigma_p(\dual{T})$, luego $\rank (T-\lambda) = X$ y $(T - \lambda)^{-1} \in L(X)$
\end{corollary}

\begin{proof}
	Como $\lambda \not \in \sigma_p(T)$ entonces por \ref{Condicion autovalor en Hilbert y compacto} existe $C > 0$ tal que $\norm{(T-\lambda)x} \geq C\norm{x}$, luego por \ref{Lemma: T es invertible si y solo si el adjunto lo es} sabemos que $ \rank T$ es cerrado y $T$ es inyectiva, pero por \ref{Relaciones de ortogonalidad} adem\'as $\rank (T-\lambda) = \ortogonal{\ker\dual{T-\lambda}} = X$. Como $T-\lambda$ es inyectiva, suryectiva y acotada entonces es inversible y adem\'as $C \norm{(T-\lambda)^{-1}y} \leq \norm{(T-\lambda \circ (T-\lambda)^{-1}y)} = \norm{y}$ por lo que $\norm{(T-\lambda)^{-1}} \leq \frac{1}{C}$. \qed
	
\end{proof}

\subsection{El teorema espectral para operadores compactos autoadjuntos}

\begin{proposition}
	\label{Autoespacio es invarinate para T y su dual}
	Sea $X$ Hilbert, $T\in L(X)$ normal y $\lambda \in \mathbb{F}$, luego $\ker(T-\lambda) = \ker \dual{T-\lambda}$ y $\ker(T - \lambda)$ es un subespacio invariante para $T$ y $\dual{T}$
\end{proposition}

\begin{proof}
	Como $T$ es normal entonces $T-\lambda$ tambi\'en lo es; afirmo entonces que $\norm{(T-\lambda)x} = \norm{\dual{T-\lambda}x}$.
	
	En efecto, $\ip{(T-\lambda)x, (T-\lambda)x} = \ip{x, \dual{T-\lambda}(T-\lambda)x} \underbrace{=}_{T-\lambda \text{ normal}} \ip{x, (T-\lambda)\dual{T-\lambda}x} = \ip{\dual{T-\lambda}x, \dual{T-\lambda}x}$.
	
	Luego $(T-\lambda)x = 0$ si y s\'olo si $\dual{T-\lambda}x = 0$.
	
	Por otro lado, si $x \in \ker(T-\lambda)$ entonces $T(x) = \lambda x \in \ker(T-\lambda)$ y $\dual{T}x = \overline{ \lambda}x \in \ker\dual{T-\lambda}$ \qed
	
\end{proof}

\begin{proposition}
	\label{Autoespacios de autovalores distintos son ortogonales}
	Sea $X$ Hilbert y $T \in L(X)$ normal, luego si $\lambda,\mu \in \sigma_p(T)$ y $\lambda \neq \mu$ entonces $\ker(T-\lambda) \perp \ker(T-\mu)$
\end{proposition}

\begin{proof}
	Sean $x \in \ker(T-\lambda), y \in \ker(T-\mu)$, notemos que:
	
	\begin{equation*}
	\begin{aligned}
		\lambda \ip{x,y} = & \ip{Tx, y} \\
		= & \ip{x, \dual{T}y} \\
		= & \ip{x, \overline{\mu}y} \\
		= & \mu \ip{x,y} \\
		\Longrightarrow & \quad \underbrace{(\lambda - \mu)}_{\neq 0} \ip{x,y} = 0 \\
...
\begin{proposition}
	\label{Autovalores de un operador autoadjunto es real}
	Sea $X$ Hilbert y $T \in L(X)$ autoadjunto, luego $\sigma_p(T) \subset \R$
\end{proposition}

\begin{proof}
	Sea $0 \neq x \in \ker(T-\lambda)$, luego $\lambda x = T(x) = \dual{T}(x) = \overline{\lambda}x$ por lo que $(\lambda - \overline{\lambda})x = 0$. \qed
\end{proof}

\begin{proposition}
	\label{Lemma: Existen autovalores no nulos en compacto autoadjunto}
	Sea $X$ Hilbert y $T \in L(X)$ autoadjunto, luego $\norm{T} = \sup\limits_{\norm{x} = 1}{\abs{\ip{Tx,x}}}$
\end{proposition}

\begin{proof}
	Por un lado es claro que $\alpha = \sup\limits_{\norm{x} = 1}{\abs{\ip{Tx,x}}} \leq \sup\limits_{\norm{x} = 1}{\norm{T}\norm{x}^2} = \norm{T}$.
	
	
	Rec\'iprocamente, primero notemos que
	
 	$\norm{T} = \sup\limits_{\norm{x}_X=1}{\norm{T(x)}} = \sup\limits_{\norm{x}_X=1}\sup\limits_{\norm{f}_{\dual{X}} =1}{\abs{f(Tx)}} \underbrace{=}_{\ref{Teorema de representacion de Riesz}} \sup\limits_{\substack{\norm{x}_X = 1 \\ \norm{y}_X=1}} {\ip{T(x),y}}$
 	
 	Asumamos que $\ip{Tx, y} \in \R$, luego por \ref{Identidad de poralizacion}:
 	
 	$$\ip{Tx,y} = \frac{1}{4} (\ip{T(x+y),x+y} - \ip{T(x-y),x-y})$$
 	
 	Por lo que $\abs{\ip{Tx,y}} \leq \frac{1}{4} (\norm{x+y}^2\alpha - \norm{x-y}^2\alpha) \underbrace{=}_{\ref{Ley del paralelogramo}} \alpha $ y por lo tanto $\norm{T} = \sup\limits_{\substack{\norm{x}_X = 1 \\ \norm{y}_X=1}} {\ip{T(x),y}} \leq \sup\limits_{\norm{x} = 1}{\abs{\ip{Tx,x}}}$. \qed
 	
\end{proof}

\begin{proposition}
	\label{Existen autovalores no nulos en compacto autoadjunto}
	Sea $X$ Hilbert y $T \in L_0(X)$ autoadjunto, luego $\pm \norm{T} \in \sigma_p(T)$
\end{proposition}

\begin{proof}
	Si $T = 0$ es claro, supongamos que $T \neq 0$, luego por \ref{Lemma: Existen autovalores no nulos en compacto autoadjunto} existe $\sett{x_n}_{n \in \N}$ tal que $\norm{x_n} = 1$ y $\ip{T(x_n), x_n} \rightarrow \lambda$ con $\abs{\lambda} = \norm{T}$. Notemos:
	
	\begin{equation*}
	\begin{aligned}
		0 \leq & \norm{(T-\lambda)x_n}^2 \\
		= & \norm{T(x_n)}^2 -2\lambda \ip{T(x_n),x_n} + \lambda^2 \\
		\leq & 2\lambda^2 -2\lambda \ip{T(x_n),x_n} \rightarrow 0
	\end{aligned}
	\end{equation*}
	
	Luego $\lambda \neq 0$ y $\norm{(T-\lambda)x_n} \rightarrow 0$, por \ref{Condicion autovalor en Hilbert y compacto} $\lambda \in \sigma_p(T)$. \qed
	
\end{proof}

\begin{theorem}[Teorema espectral para operadores compactos autoadjuntos]
	Sea $X$ Hilbert y $T \in L_0(X)$ autoadjunto, entonces $T$ admite numerables distintos autovalores. Es m\'as si $\sett{\lambda_1, \dots, \lambda_n, \dots} \subset \sigma_p(T) \cap \R^{\ast}$ y $P_n = P_{\ker(T-\lambda_n)}$ luego $P_nP_n = P_nP_m = 0$ si $n \neq m$ y vale:
...
\begin{proof}
	Por \ref{Existen autovalores no nulos en compacto autoadjunto} existe $\lambda_1 \in \sigma_p(T)$ con $\abs{\lambda_1} = \norm{T}$, sea $K_1 = \ker (T-\lambda_1)$, $P_1 = P_{K_1}$ y $X_2 = \ortogonal{K_1}$; por \ref{Autoespacio es invarinate para T y su dual} $K_1$ y por ende $X_2$ son subespacios invariantes para $T$, si consideramos $T_1 = T\vert_{X_2}$ entonces $T_2 \in L_0(X_2)$ y es autoadjunto. 
	
	Iterando existe $\lambda_2 \in \sigma_p(T_2)$ con $\abs{\lambda_2} = \norm{T_2}$, sea $K_2 = \ker (T-\lambda_2)$, $P_2 = P_{K_2}$ y $X_3 = \ortogonal{K_1 \oplus K_2}$, notemos que $\norm{T_2} \leq \norm{T}$ y que $\abs{\lambda_2} \leq \abs{\lambda_1}$. Luego por inducci\'on existe $\sett{\lambda_n} \subset \R$ (\ref{Autovalores de un operador autoadjunto es real}) tal que:
	
	\begin{itemize}
		\item $\abs{\lambda_1} \geq \abs{\lambda_2} \geq \dots$
		\item Si $K_n = \ker(T-\lambda_n)$ entonces $\abs{\lambda_{n+1}} = \norm{T \vert_{\ortogonal{K_1 \oplus \dots \oplus K_n}}}$
	\end{itemize}
	 
	
	Es claro que existe $\alpha \geq 0$ tal que $\lim \abs{\lambda_n} = \alpha$, afirmo que $\alpha = 0$.
	
	En efecto, sea $k_n \in K_n$ tal que $\norm{k_n} = 1$, luego como $T$ es compacto existe $\sett{k_{n_j}}_{j \in \N}$ subsucesi\'on y $x \in X$ tal que $T(k_{n_j}) \xrightarrow{\norm{.}_X} x$, pero $\norm{T(k_{n_j} - T(k_{n_m}))} \underbrace{=}_{\ref{Autoespacios de autovalores distintos son ortogonales}} \lambda_{n_j}^2 + \lambda_{n_m}^2 \geq 2\alpha^2$, luego $\alpha = 0$.
	
	Finalmente analicemos:
	
	\begin{equation*}
	(T - \sum\limits_{i=1}^{n} \lambda_i P_i)(x) = \left\lbrace \begin{array}{cccc}
	T(x) - \lambda_jx & = & 0 & \text{si } x \in K_j \quad 1 \leq j \leq n \\
	T(x) - 0 & = & T(x) & \text{si } x \in \ortogonal{K_1 \oplus \dots \oplus K_n}
	\end{array}\right.
	\end{equation*}
	 
	Luego como $ \ortogonal{K_1 \oplus \dots \oplus K_n}$ es invariante para $T$:
	
	\begin{equation*}
	\begin{aligned}
		\norm{T - \sum\limits_{i=1}^{n} \lambda_i P_i} = & \norm{T \vert_{\ortogonal{K_1 \oplus \dots \oplus K_n}}} \\
		= & \abs{\lambda_{n+1}} \rightarrow 0
	\end{aligned}
	\end{equation*}
	\qed
\end{proof}
...
\begin{corollary}
	Con la misma notaci\'on de antes:
	
	\begin{enumerate}
		\item $\ker T = \ortogonal{\bigvee \left\lbrace P_nX\right\rbrace} = \ortogonal{\rank T}$
		\item $P_n \in L_{00}(X)$
		\item $\norm{T} = \sup {\abs{\lambda_n}}$ y $\lambda_n \rightarrow 0$
	\end{enumerate}
	
\end{corollary}

\begin{corollary}
	Sea $X$ Hilbert y $T \in L_0(X)$ autoadjunto, luego existe $\sett{\mu_n} \subset \R$ y $\sett{e_n} \subset \ortogonal{\ker T}$ base ortogonal tal que:
	
	\begin{equation}
		T(x) = \Bigsum{n \in \N}{\mu_n \ip{x,e_n}e_n}
	\end{equation}
	
\end{corollary}

\begin{corollary}
	Sea $X$ Hilbert y $T \in L_0(X)$ autoadjunto e inyectivo, entonces $X \simeq l^2$.
\end{corollary}
