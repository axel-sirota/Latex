...
%\begin{frame}{Convergencia en L1}
%\begin{theorem}
%	Sea $F: \R^d \rightarrow \R$ tal que $F \in C^1$ la funci\'on objetivo , $F$ es \textit{L-Lipschitz} y supongamos adem\'as que $g$ tiene \textit{varianza acotada}. Luego el algoritmo \textit{descenso estoc\'astico de gradiente generalizado} con incremento fijo $0  < \alpha_k = \alpha \leq \dfrac{\mu}{LM_G} $ cumple:
%	
%	\begin{equation*}
...
%\begin{frame}{Idea de la demostraci\'on}
%Sea $G(w) := \norm{\nabla F(w)}_2^2$ y sea $L_G$ la constante de Lipschitz de $\nabla G(w) = 2 \nabla^2 F(w) \nabla F(w)$, luego:
...
%\begin{frame}{Recuerdo: Quasi Martingalas Parte 1}
%\begin{definition}
...
%\end{frame}
%
%\begin{frame}{Recuerdo: Quasi Martingalas Parte 2}
%\begin{footnotesize}
%\begin{definition}
...
%	
%	\begin{equation}
%	\mathbb{V}ar_n(X) = \sup\limits_{J \subset \sett{1, \dots, n}} \sett{\expectation{\sum\limits_{\substack{i_k \in J \\ 1 \leq k \leq \abs{J}}}{\abs{\conditionalExpectation{X_{i_{k+1}} - X_{i_k}}{\F_{i_{k}}}}}}} < \infty
%	\end{equation}
%	
%\end{definition}
%
...
%\end{footnotesize}
%\end{frame}
%\begin{frame}{Teorema de convergencia de cuasi-martingalas}
%\begin{theorem}
