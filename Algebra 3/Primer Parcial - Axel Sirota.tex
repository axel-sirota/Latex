...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\begin{proof}
			\textcolor{blue}{
			En efecto, como $\xi$ es una ra\'iz de $\Phi_n$ entonces $f(\xi, F) = \Phi_n$ pues es irreducible, m\'onico y anula; luego $[F[\xi]:F] = \varphi(n) = gr(\Phi_n)$ y la vuelta es por definici\'on. Como $\quotient{F[\xi]}{F}$ es Galois ($p \not\vert n$), sabemos que $\varphi(n) = \abs{Gal\left(\quotient{F[\xi]}{F}\right)} = \abs{(\quotient{\Z}{n\Z}} = \varphi(n)$, luego la inyecci\'on $Gal\left(\quotient{F[\xi]}{F}\right) \inc \left(\quotient{\Z}{n\Z}\right)^*$ es un isomorfismo de grupos pues de \'Algebra 2 sabemos que un homomorfismo inyectivo (esto lo sabemos de la te\'orica) entre grupos de igual orden es isomorfismo, y la vuelta es trivial. Finalmente de la teor\'ia de cuerpos finitos sabemos que siempre:
			\begin{equation*}
				f\left(\xi, \mathbb{F}_p\right) = \prod\limits_{i=1}^{k}{x-\xi^{p^i}}
			\end{equation*}
			donde $k = [{\mathbb{F}_p[\xi]}:{\mathbb{F}_p}]$. Luego sabemos que $ f\left(\xi, \mathbb{F}_p\right) \vert \Phi_n$ y van a ser iguales si y s\'olo si sus ra\'ices son iguales. Como las ra\'ices de $\Phi_n$ son de la forma $\xi^a$ para todo $a \leq n$ tal que $(a, n) = 1$ entonces las ra\'ices son iguales si para todo $a \leq n$ tal que $(a, n) = 1$ existe un \'unico $\widetilde{i} \leq k$ tal que $\xi^{a} = \xi^{p^{\widetilde{i}}}$ .Esto vimos que pasa si y s\'olo si $a = p^{\widetilde{i}} \ \mod(n)$.  Luego esto pasa si y s\'olo si para cada elemento $a \in \left(\quotient{\Z}{n\Z}\right)^*$ se puede escribir de la forma $p^{i}$ para un \'unico $i \leq k$ lo que pasa si y s\'olo si $p$ es generador de $\left(\quotient{\Z}{n\Z}\right)^*$.}  \qed
		\end{proof}
	...
	\begin{itemize}
		...
		\item \textcolor{red}{Dijimos que no habia que probarlo} \qed
